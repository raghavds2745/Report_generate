...
with st.sidebar:
    ...
    data_source = st.radio(
        "Choose data source:",
        options=["Browse network path","Upload CSV Files"],
        index=0,
        help="Select whether to upload your own files or use network"
    )
    ...
    if 'current_path' not in st.session_state:
        st.session_state.current_path = "\\\\infranas01plp\\AUT_INFRA\\AIAI\\Replay"
    if 'prev_csv_path' not in st.session_state:
        st.session_state.prev_csv_path = None
    if 'new_csv_path' not in st.session_state:
        st.session_state.new_csv_path = None
    prev_file = None
    new_file = None
    if data_source =="Browse network path":
            ...
            if csv_files:
                ...
                for csv_file in sorted(csv_files):
                    ...
                    with col1:
                        st.write(f"📊 {csv_file}")
                    with col2:
                        if st.button("Prev", key=f"prev_{csv_file}"):
                            st.session_state.prev_csv_path = os.path.join(st.session_state.current_path, csv_file)
                            st.success(f"✅ Previous CSV: {csv_file}")
                    with col3:
                        if st.button("New", key=f"new_{csv_file}"):
                            st.session_state.new_csv_path = os.path.join(st.session_state.current_path, csv_file)
                            st.success(f"✅ New CSV: {csv_file}")
            else:
                st.info("No CSV files in this directory")
        else:
            st.error(f"Path not accessible: {st.session_state.current_path}")
    except PermissionError:
        st.error(f"Permission denied: {st.session_state.current_path}")
    except Exception as e:
        st.error(f"Error: {str(e)}")  
    st.subheader("📋 Selected Files")
    col1, col2 = st.columns(2)
    with col1:
        if st.session_state.prev_csv_path:
            st.info(f"✅ Previous:\n`{os.path.basename(st.session_state.prev_csv_path)}`")
        else:
            st.warning("⏳Previous_CSV")
    with col2:
        if st.session_state.new_csv_path:
            st.info(f"✅ New:\n`{os.path.basename(st.session_state.new_csv_path)}`")
        else:
            st.warning("⏳ New_CSV")
    
    prev_csv = st.session_state.prev_csv_path
    new_csv = st.session_state.new_csv_path

    # else:
    # # Upload mode (original)
    # col1, col2 = st.columns(2)
    # with col1:
    #     prev_csv = st.file_uploader("Upload Previous CSV", type="csv")
    # with col2:
    #     new_csv = st.file_uploader("Upload New CSV", type="csv")
...
if prev_csv and new_csv:
    try:
        ...
        # Verify required columns exist
        df_prev.rename(columns={"Question": "user_input"}, inplace=True)
        df_new.rename(columns={"Question": "user_input"}, inplace=True)
        required_cols = ['user_input', 'answer_relevancy', 'answer_correctness', 
                        'Precision', 'Recall', 'Reranker Score', 'F1 Score']
...
# def compare_csvs(prev_file, new_file, merge_key="user_input"):
...
#         drop_detected = False
#         row_out = {
#             "Question": row.get(merge_key, ""),
#             "Reference response": row.get("reference_prev", row.get("reference_new", "")),
#             "Response_prev": row.get("response_prev", ""),
#             "Response_new": row.get("response_new", "")
#         }

#         for metric in metrics:
#             old = row.get(metric + "_prev", None)
#             new = row.get(metric + "_new", None)

#             row_out[metric + "_prev"] = old
#             row_out[metric + "_new"] = new
#             row_out[metric + "_drop"] = (old - new) if (pd.notna(old) and pd.notna(new)) else None
...
# # Streamlit UI
# # --------------------------------------------------
# st.set_page_config(page_title="LLM Replay Comparison", layout="wide")
# st.title("📊 LLM Replay Comparison")
...
# if "current_path" not in st.session_state:
#     st.session_state.current_path = r"\\infranas01plp\AUT_INFRA\AIAI\Replay"
...
# # ----------- Input method (UPDATED) ---------------
# col1, col2 = st.columns(2)
# with col1:
#     if st.button("📁 Browse Network Path"):
...
#             folders = [
#                 i for i in items
#                 if os.path.isdir(os.path.join(st.session_state.current_path, i))
#             ]
#             csv_files = [i for i in items if i.lower().endswith(".csv")]
...
#                         st.write(csv)
#                     with c2:
#                         if st.button("Prev", key=f"prev_{csv}"):
#                             st.session_state.prev_csv_path = os.path.join(
#                                 st.session_state.current_path, csv
#                             )
#                     with c3:
#                         if st.button("New", key=f"new_{csv}"):
#                             st.session_state.new_csv_path = os.path.join(
#                                 st.session_state.current_path, csv
#                             )
#             else:
...
# if prev_csv and new_csv:
#     try:
#         prev_df_check = pd.read_csv(prev_csv)
#         new_df_check = pd.read_csv(new_csv)

#         prev_df_check.columns = prev_df_check.columns.str.strip()
#         new_df_check.columns = new_df_check.columns.str.strip()

#         common_cols = list(set(prev_df_check.columns) & set(new_df_check.columns))
...
#             df = compare_csvs(prev_csv, new_csv, merge_key)

#             if df.empty:
#                 st.success("✅ No significant drops found!")
#             else:
...
#                     agg_data.append({
#                         "Metric": metric,
#                         "Avg Drop": df[metric + "_drop"].mean(skipna=True),
#                         "Total Drop": df[metric + "_drop"].sum(skipna=True),
#                         "Count": df[metric + "_drop"].count()
#                     })

#                 agg_df = pd.DataFrame(agg_data)
#                 st.subheader("📉 Metric Drop Analysis")
#                 st.dataframe(agg_df, use_container_width=True)

#                 fig1 = px.bar(
#                     agg_df, x="Metric", y="Avg Drop",
#                     title="Average Drop per Metric", text_auto=".3f"
#                 )
#                 st.plotly_chart(fig1, use_container_width=True)

#                 melted = df.melt(
#                     id_vars=["Question"],
#                     value_vars=[m + "_drop" for m in metrics],
#                     var_name="Metric",
#                     value_name="Drop"
#                 )
#                 melted["Metric"] = melted["Metric"].str.replace("_drop", "")

#                 fig2 = px.box(
#                     melted, x="Metric", y="Drop",
#                     title="Distribution of Drops per Metric"
#                 )
#                 st.plotly_chart(fig2, use_container_width=True)

#                 fig3 = px.line(
#                     melted, x="Question", y="Drop",
#                     color="Metric", markers=True,
#                     title="Drop Trend by Question"
#                 )
